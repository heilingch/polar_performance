...
class SwellImpactModel:
    ...
    def _calculate_beating_degradation(self, twa, tws, wave_height, wave_period):
        ...
        # Calculate wave height to boat length ratio (higher = worse performance)
        wave_to_length_ratio = wave_height / length
        
        # Base degradation from wave height - more severe for upwind sailing
        # Boats typically lose significant speed upwind in waves
        base_degradation = 1.0 - (wave_to_length_ratio * 0.6)  # Increased from 0.4
        ...
        # Progressive reduction with increasing wind
        wind_factor = max(0.6, 1.0 - (0.015 * tws))  # 1.5% degradation per knot, minimum 60%
        ...
        if wave_steepness > 0.05:  # Moderately steep
            degradation_factor *= (1.0 - (wave_steepness - 0.05) * 3)  # Increased from 2
    ...
    def _calculate_reaching_degradation(self, twa, tws, wave_height, wave_period):
        ...
        # Wave to length ratio
        wave_to_length_ratio = wave_height / length
        
        # Base degradation - less severe than beating
        base_degradation = 1.0 - (wave_to_length_ratio * 0.3)
        ...
        resonance_factor = 1.0 - 0.2 * max(0, 1.0 - (period_diff / est_roll_period))
        ...
        if twa < 90:
            ...
            angle_factor = 1.0 - (0.2 * (90 - twa) / 30)
    ...
    def _calculate_running_degradation(self, twa, tws, wave_height, wave_period):
        ...
        # This is more realistic for following seas performance
        if 1.0 < wavelength_ratio < 2.0 and wave_to_length_ratio < 0.15:
            # Potential improvement from surfing when wavelength is suitable
            # More generous thresholds than original model
            base_degradation = 1.0 + (wave_to_length_ratio * 0.3)
        else:
            # Less degradation for unsuitable wavelengths than original model
            base_degradation = 1.0 - ((wave_to_length_ratio - 0.05) * 0.2)
        ...
        # Wind-wave interaction - modified to be less punitive
        # Higher winds + bigger waves = risk of broaching, but reduced impact
        # From the original model
        wind_wave_factor = 1.0 - (0.005 * tws * wave_to_length_ratio * 10)  # Reduced from 0.01
    ...
    def analyze_factors(self, twa, tws, wave_height, wave_period):
        ...
        if 0 <= twa <= 60:  # Beating
            ...
            base_degradation = 1.0 - (wave_to_length_ratio * 0.6)
            boat_factor = min(1.2, (0.8 + 0.1 * length_beam_ratio + 0.1 * draft + 0.1 * (displacement_length / 100)))
            wind_factor = max(0.6, 1.0 - (0.015 * tws))
            period_factor = max(0.7, min(1.0, wave_period / 8))
            
            result["factors"]["mode"] = "beating"
            ...
            if wave_steepness > 0.05:
                steepness_factor = (1.0 - (wave_steepness - 0.05) * 3)
            ...
            result["factors"]["custom_factor"] = self.custom_beating_factor
            ...
            result["factors"]["raw_degradation"] = degradation
            result["factors"]["custom_factor"] = self.custom_reaching_factor
            ...
            tracking_factor = 0.9 + (0.1 * min(1.0, lb_ratio / 3.5))
            weight_factor = 0.9 + (0.1 * min(1.0, weight / 10000))
            wind_wave_factor = 1.0 - (0.005 * tws * wave_to_length_ratio * 10)
            period_factor = max(0.8, min(1.0, wave_period / 6))
            ...
            result["factors"]["raw_degradation"] = degradation
            result["factors"]["custom_factor"] = self.custom_running_factor
            
        # Apply custom factor
        final_degradation = max(0.1, min(1.0, degradation * result["factors"]["custom_factor"]))
